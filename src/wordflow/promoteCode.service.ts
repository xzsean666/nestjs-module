...
export class PromoteCodeService {
  ...
  private async initializeDefaultPromoteCodes(): Promise<void> {
    try {
      for (const promoteCode of defaultPromoteCodes) {
  ...
  async validateAndApplyPromoteCode(
    ...
    // 检查是否过期
    if (promoteCode.expiryDate && new Date() > promoteCode.expiryDate) {
      return {
        isValid: false,
        message: '优惠码已过期',
      };
  ...
  async getActivePromoteCodes(): Promise<PromoteCode[]> {
    ...
    for (const [key, value] of allCodes) {
      ...
      if (
        ...
        (!promoteCode.expiryDate || new Date() <= promoteCode.expiryDate)
      ) {
  ...
  async getPromoteCodeStats(): Promise<{
    ...
    for (const [key, value] of allCodes) {
      ...
      if (
        promoteCode.isActive &&
        (!promoteCode.expiryDate || now <= promoteCode.expiryDate)
      ) {
        active++;
      } else {
        expired++;
